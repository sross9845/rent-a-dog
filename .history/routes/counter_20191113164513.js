...
router.get('/', (req, res) => {
    ...
    User.find({} , (err, users) => {
        ...
        for (let i=0; i < myDogNameList.length; i++) {
            ...
            var helper
            if (0 >= i <= 9) {
                iterator = Math.abs(i-result.length+1)
                // helper = 
            } else {
                iterator = Math.abs(i-result.length)
            }
            console.log(myDogNameList[i], result[iterator][0])
            if (myDogNameList[i] === result[iterator][0] || myDogNameList[i] === result[iterator][0]) {
                result[iterator].splice(i, 1, myDogNameList[i+1])
