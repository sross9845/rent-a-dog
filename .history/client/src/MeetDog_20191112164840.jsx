...
class MeetDog extends Component {
    ...
    render() { 
        if (this.state.dogList) {
            ...
            var mappedDogs = this.state.dogList.map((ele, id) => <ul className='ulStyle'><li key={id}>{ele.name}</li><li>{ele.status}</li><li>{ele.email}</li><li>{ele.phone}</li><li>{ele.state}</li><li>{ele.city}</li></ul>)
        } else {
